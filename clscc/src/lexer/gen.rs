...
// This is inspired by / based on `monkey-rust`
...
macro_rules! syntax {
    ($func:ident, $tag:literal, $typ:ident, $tok:ident) => {
        pub fn $func(s: Span<'_>) -> IResult<Span<'_>, Token<'_>> {
            map(tag($tag), |s: Span<'_>| Token {
                variant: TokenVariant::$typ($typ::$tok),
                string_repr: $tag,
                line_num: s.location_line() as usize,
                line: s.extra.lines().nth(s.location_line() as usize - 1).unwrap(),
                col_num: s.get_utf8_column(),
            })(s)
        }
    };
}
...
pub fn lex_punctuator(inp: Span<'_>) -> IResult<Span<'_>, Token<'_>> {
    alt((
        ...
        alt((punc_comma, punc_hash, punc_hashhash)),
    ))(inp)
}
